import { financeCommand } from "./finance.js";
...
async function execute(db, actor, input) {
  if (input.action?.startsWith("finance."))
    return financeCommand(db, actor, input);
  if (input.action?.startsWith("catalog."))
    return catalogCommand(db, actor, input);
  const resultId = id();
  switch (input.action) {
    case "product.create":
      allow(actor, workers);
